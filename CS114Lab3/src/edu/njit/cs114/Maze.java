...
public class Maze {
    ...
    public boolean findMazePathAux(int x, int y, int destCol, int destRow) {
        ...
        if (x == destCol && y == destRow) {
            ...
            // Mark the current cell as visited by setting its color to TEMPORARY
            maze.recolor(x, y, TEMPORARY);
            // Try moving in each of the four possible directions
            if (x > 0 && maze.getColor(x-1, y) == NON_BACKGROUND && findMazePath(x-1, y, destCol, destRow)) {
                // If a path is found, mark the current cell as part of the path and return true
                maze.recolor(x, y, PATH);
                return true;
            }
            if (x < maze.getNCols()-1 && maze.getColor(x+1, y) == NON_BACKGROUND && findMazePath(x+1, y, destCol, destRow)) {
                maze.recolor(x, y, PATH);
                return true;
            }
            if (y > 0 && maze.getColor(x, y-1) == NON_BACKGROUND && findMazePath(x, y-1, destCol, destRow)) {
                maze.recolor(x, y, PATH);
                return true;
            }
            if (y < maze.getNRows()-1 && maze.getColor(x, y+1) == NON_BACKGROUND && findMazePath(x, y+1, destCol, destRow)) {
                maze.recolor(x, y, PATH);
                return true;
            }
